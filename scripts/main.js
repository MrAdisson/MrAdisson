  ...
  .addEventListener('click', (e) => {
    if (!timelineLeftMenu.isActive()) {
      ...
      if (leftMenuState) {
        ...
        audio.play();
        audio.volume = 0.4;
        timelineLeftMenu.to('#mainContainer', {
          x: 0,
          duration: 0.5,
        });
        timelineLeftMenu.to('#linkMenu', { x: 0, duration: 0.5 });
      } else {
        var audio = new Audio('/res/audio/menuOpen.wav');
        audio.play();
        audio.volume = 0.4;
        ...
        timelineLeftMenu.to('#linkMenu', { x: -50, duration: 0.5 });
        timelineLeftMenu.to('#mainContainer', {
          x: -20,
          duration: 1,
        });
...
for (let i = 0; i < linkList.length; i++) {
  linkList[i].addEventListener('mouseenter', (e) => {
    gsap.to(circle, { backgroundColor: 'purple', duration: 0.4 });
    gsap.to(circle, { width: 60, height: 60, duration: 0.4 });
  ...
  linkList[i].addEventListener('mouseleave', (e) => {
    gsap.to(circle, { backgroundColor: circleColor, duration: 0.4 });
    gsap.to(circle, { width: 15, height: 15, duration: 0.4 });
  });
}
